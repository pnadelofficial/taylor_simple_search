        ...
        if choice == 'national_archive_index':
            ...
            if cat_choice in groups:
                ...
                for i, res in enumerate(hits[st.session_state.start:st.session_state.start+st.session_state.to_see]):
                    ...
                    if on:
                        full = OLDutils.display_results(i, r, data, searches, display_date=False)
                    else:
                        full = OLDutils.display_results(i, r, data, searches, display_date=True)
            ...
            for i, r in enumerate(results[st.session_state.start:st.session_state.start+st.session_state.to_see]):
                ...
                full_with_r = OLDutils.display_results(i, r, data, searches)
...
if export_as_pdf_page:
    OLDutils.export_as_pdf_page(text_for_save, query_str, choice)

if export_as_pdf_full:
    OLDutils.export_as_pdf_full(results, query_str, choice, ix, cats, q)